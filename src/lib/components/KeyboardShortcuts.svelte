...
<button
  class="fixed right-5 bottom-5 z-9000 flex h-9.5 w-9.5 cursor-pointer items-center justify-center rounded-full border border-ink-660 bg-ink-820 text-base font-semibold text-[#9a9ab0] shadow-[0_4px_14px_rgba(0,0,0,0.4)] transition-all duration-150 hover:-translate-y-px hover:border-accent-700 hover:bg-[#2a2a44] hover:text-white"
    ...
    {#each shortcuts as s}
      <li class="flex items-center gap-[0.9rem]">
        <span class="flex min-w-17.5 shrink-0 gap-[0.3rem]">
          {#each s.keys as k}
            <kbd
              class="min-w-[1.2rem] rounded-md border border-b-2 border-ink-660 bg-ink-870 px-[0.55rem] py-[0.2rem] text-center font-[inherit] text-[0.78rem] text-[#d0d0e0]"
              >{k}</kbd>
          {/each}
        </span>
