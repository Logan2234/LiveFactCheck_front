...
<div
  class="flex h-8 items-center gap-0.5 rounded-full border border-edge bg-surface p-0.5"
  role="group"
  ...
  {#each OPTIONS as opt (opt.value)}
    <button
      ...
      class={[
        "flex h-full cursor-pointer items-center gap-1 rounded-full px-3 text-xs transition-colors",
