...
<div class="flex h-[calc(100vh-160px)] min-h-125 flex-col">
  <div
    class="flex items-center rounded-t-[10px] border border-b-0 border-ink-700 bg-ink-840 px-4 py-3 text-[0.85rem] text-ash-600">
    <div class="flex flex-1 items-center gap-2">
      <div class="flex h-8 w-8 items-center justify-center rounded-full bg-ink-700 text-base">
        🎙
      </div>
      <span>Speaker</span>
    </div>
    <div class="flex-none px-4 text-xs text-ash-750">vs</div>
  ...
  <div
    class="flex flex-1 flex-col gap-3 overflow-y-auto rounded-b-[10px] border border-t-0 border-ink-700 bg-[#0f0f1e] p-4">
    {#if feed.length === 0}
      <div class="flex flex-1 items-center justify-center text-[0.9rem] text-ash-750">
      ...
      {#each feed as item (item.kind === "claim" ? item.id : item.timestamp + item.text)}
        {#if item.kind === "transcript"}
          ...
          <div class="flex max-w-[70%] items-end gap-2 self-start">
            <div
              class="flex h-7 w-7 shrink-0 items-center justify-center self-end rounded-full bg-ink-770 text-[0.9rem]">
              🎙
            </div>
            <div
              class="relative max-w-full rounded-2xl rounded-tl-sm bg-[#252540] px-[0.9rem] py-[0.6rem]">
              <p class="mt-0 mb-1 text-[0.9rem] leading-normal text-fog-250">{item.text}</p>
              <span class="block text-right text-[0.72rem] tabular-nums text-ash-700"
        ...
        {:else}
          ...
          <div class="flex max-w-[70%] flex-row items-end gap-2 self-end">
            <div
              ...
              <p class="mt-0 mb-1 text-[0.9rem] leading-normal text-fog-250 italic">
                « {item.text} »
              </p>
              {#if item.explanation && item.status !== "pending"}
                <p class="mt-1 mb-[0.1rem] text-[0.82rem] leading-[1.4] text-ash-500">
                  {item.explanation}
                </p>
              {/if}
              <span class="block text-right text-[0.72rem] tabular-nums text-ash-700"
