...
<div class="overflow-hidden rounded-lg border border-surface bg-surface-term font-term">
  <div
      ...
      {#each filterKeys as { key, label } (key)}
        <button
          class={[
            ...
            $claimFilter === key ? "text-accent-light" : "text-fg-muted hover:text-fg"
          ]}
  ...
  <div class="flex max-h-[calc(100vh-260px)] flex-col gap-1.5 overflow-y-auto px-4 py-3">
    {#if $log.length === 0}
      <div class="flex items-baseline gap-3 text-sm leading-normal">
        ...
        <span class="w-[1ch] shrink-0 text-center font-bold text-fg-faint">_</span>
        <span class="flex-1 wrap-break-word text-fg-muted italic">waiting for audio input...</span>
      </div>
    {:else}
      {#each $log as item (item.kind + "-" + (item.kind === "claim" ? item.claim.id : item.timestamp))}
        {#if item.kind === "transcript"}
          <div class="flex items-baseline gap-3 text-sm leading-normal">
            ...
            <span class="w-[1ch] shrink-0 text-center font-bold text-fg-faint">»</span>
            <span class="flex-1 wrap-break-word text-fg-muted italic">{item.text}</span>
          </div>
        {:else}
          <div
            ...
            <span class="flex-1 wrap-break-word text-fg">
              ...
              {#if item.claim.explanation}
                <span class="text-sm text-fg-muted"> — {item.claim.explanation}</span>
              {/if}
            </span>
          </div>
        {/if}
